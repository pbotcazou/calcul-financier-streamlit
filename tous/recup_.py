...
def format_boolean_icon(val):
    # Convertit en string, en minuscules, sans espace
    val_str = str(val).strip().lower()

    # Tous les cas considérés comme "vrai"
    truthy_values = {"true", "1", "oui", "yes", "vrai"}

    is_true = val_str in truthy_values

    icon = "✔" if is_true else "✖"
    icon_color = "green" if is_true else "red"
...
def display_customer(customer_row):
    with st.container():
        ...
        with col2:
            ...
            st.markdown(f"<strong>Caisse :</strong>{format_boolean_icon(customer_row['syd_caisse'])}", unsafe_allow_html=True)
            st.markdown(
